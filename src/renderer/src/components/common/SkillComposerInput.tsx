  ...
  return (
    <Tooltip.Root delayDuration={120}>
      ...
      <Tooltip.Portal>
        <Tooltip.Content
          sideOffset={8}
          className="z-[80] max-w-[280px] rounded-2xl border border-slate-200 bg-white px-3 py-2 text-xs leading-5 text-slate-700 shadow-[0_16px_40px_rgba(15,23,42,0.14)]"
        >
          {description || '暂无描述'}
        </Tooltip.Content>
      </Tooltip.Portal>
              ...
              {filteredSkills.length > 0 ? (
                <div className="max-h-64 overflow-y-auto p-2">
                  {filteredSkills.map((skill, index) => (
                    <SkillTooltip key={skill.id} description={skill.description}>
                      <button
                        type="button"
                        onMouseDown={(event) => {
                      ...
                      >
                        ...
                        <Command size={14} className="flex-shrink-0 text-slate-300" />
                      </button>
                    </SkillTooltip>
                  ))}
                </div>
        ...
        {selectedSkills.length > 0 && (
          <div className="mb-2 flex flex-wrap gap-2">
            {selectedSkills.map((skill) => (
              <SkillTooltip key={skill.id} description={skill.description}>
                <span className="inline-flex items-center gap-2 rounded-2xl border border-sky-200 bg-sky-50 px-3 py-1.5 text-sm text-sky-700">
