...
import cursorIcon from '../../assets/open-apps/cursor.png'
import finderIcon from '../../assets/open-apps/finder.svg'
import ghosttyIcon from '../../assets/open-apps/ghostty.png'
import golandIcon from '../../assets/open-apps/goland.svg'
import intellijIcon from '../../assets/open-apps/intellij.svg'
import iterm2Icon from '../../assets/open-apps/iterm2.png'
import riderIcon from '../../assets/open-apps/rider.svg'
import terminalIcon from '../../assets/open-apps/terminal.png'
import vscodeIcon from '../../assets/open-apps/vscode.png'
import warpIcon from '../../assets/open-apps/warp.png'
import xcodeIcon from '../../assets/open-apps/xcode.png'

const OPEN_APP_ICONS: Record<string, string> = {
  vscode: vscodeIcon,
  cursor: cursorIcon,
  finder: finderIcon,
...
export function ProjectOpenWithControl({ cwd = '', className, alwaysVisible = false }: ProjectOpenWithControlProps) {
  ...
  return (
    <div ref={rootRef} className={cn('titlebar-no-drag relative', className)}>
      <div className="inline-flex h-8 items-center overflow-hidden rounded-[12px] border border-border bg-card text-muted-foreground shadow-[0_6px_16px_rgba(15,23,42,0.06)]">
        <button
          type="button"
          onClick={() => void handleOpenProjectWithApp()}
          disabled={!selectedOpenApp || !cwd || loading}
          className="inline-flex h-8 w-10 items-center justify-center transition-colors hover:bg-muted/70 hover:text-foreground disabled:cursor-not-allowed disabled:opacity-45"
        ...
        <button
          ...
          className="inline-flex h-8 w-8 items-center justify-center border-l border-border transition-colors hover:bg-muted/70 hover:text-foreground disabled:cursor-not-allowed disabled:opacity-45"
      ...
      {menuOpen ? (
        <div className="absolute right-0 top-[calc(100%+8px)] z-50 w-[158px] overflow-hidden rounded-[14px] border border-border bg-card py-1 shadow-[0_14px_34px_rgba(15,23,42,0.13)]">
          {openApps.map((app) => (
            <button
              key={app.id}
              type="button"
              ...
              className={cn(
                'flex w-full items-center gap-2 px-3 py-1.5 text-left text-sm transition-colors hover:bg-muted/70',
                selectedOpenApp?.id === app.id && 'bg-muted/70 text-foreground'
              )}
            >
              <ProjectOpenAppIcon app={app} size="md" />
              <span className="min-w-0 flex-1 truncate text-[13px] text-foreground">{app.name}</span>
            </button>
          ))}
        </div>
      ) : null}
...
function ProjectOpenAppIcon({ app, size }: { app: ProjectOpenApp | null; size: 'sm' | 'md' }) {
  const boxClass = size === 'sm' ? 'h-5 w-5 rounded-[5px]' : 'h-4 w-4'
  const iconSrc = app ? OPEN_APP_ICONS[app.id] : ''

  if (iconSrc) {
    return <img src={iconSrc} alt="" className={cn('object-contain', boxClass)} aria-hidden="true" />
  }

  return (
    <span
      className={cn('flex items-center justify-center bg-muted/80 text-muted-foreground', size === 'sm' && 'rounded-md', boxClass)}
      aria-hidden="true"
    >
