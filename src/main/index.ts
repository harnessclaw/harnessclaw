...
function findAppBundleByBundleId(bundleId: string | undefined): string | undefined {
  if (process.platform !== 'darwin' || !bundleId) return undefined
  const escapedBundleId = bundleId.replace(/\\/g, '\\\\').replace(/"/g, '\\"')
  const result = spawnSync('/usr/bin/mdfind', [`kMDItemCFBundleIdentifier == "${escapedBundleId}"`], {
    encoding: 'utf8',
    timeout: 1600,
  })
  if (result.status !== 0) return undefined
  ...
  return result.stdout
    ...
    .filter(Boolean)
    .find((candidate) => {
      if (!candidate.endsWith('.app') || !existsSync(candidate)) return false
      return readAppBundleInfo(candidate).bundleId === bundleId
    })
}
...
function resolveKnownProjectOpenApps(): ProjectOpenAppInfo[] {
  return KNOWN_PROJECT_OPEN_APPS.flatMap((definition) => {
    ...
    let appPath = definition.appPaths
      ?.map(expandUserPath)
      .find((candidate) => {
    ...
    if (!appPath) {
      appPath = findAppBundleByBundleId(definition.bundleId)
      if (appPath) detectedBy.push('appBundle')
    }
